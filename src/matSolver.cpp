...
{
    Eigen::MatrixXcd result(n * n, n * n);
    result.setZero();

// Parallel region for computing the first result matrix
#pragma omp parallel
    {
        Eigen::MatrixXcd local_result = Eigen::MatrixXcd::Zero(n * n, n * n);
...
#pragma omp critical
    ...
    Eigen::MatrixXcd result2(n * n, n * n);
    result2.setZero();

// Parallel region for computing the second result matrix
#pragma omp parallel
    {
        Eigen::MatrixXcd local_result2 = Eigen::MatrixXcd::Zero(n * n, n * n);
...
#pragma omp critical
    ...
    Eigen::EigenSolver<Eigen::MatrixXcd> solver(functionMat);
